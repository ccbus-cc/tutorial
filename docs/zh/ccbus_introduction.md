...
**5. 发币要付多少钱？**  
不同区块链的发币费用不一样，通常以该链本身代币支付。例如你要在币安链发币，需要支付 BNB；在 Solana 链发币，需要支付 SOL。具体收费标准参考：[CCBus 收费标准](/zh/price)。
...
- [PumpFun 发币教程](/zh/sol/createpump)  
- [BonkFun 发币教程](/zh/sol/createbonk)
...
- Solana 权限管理教程：</zh/sol/control>  
- Sui 权限管理教程：</zh/sui/control>
...
- X Layer 需要手动开源，教程：[X Layer 验证教程](/zh/question/xlayer-verified)；  
- Solana、Sui、TON 暂时无需开源；  
- TRON 波场链需要手动开源，教程：[TRON 验证教程](/zh/tron/verify)。

**4. 发币后如何才能交易？**  
需要为代币创建流动性资金池（pool）。不同链与不同交易平台加池方式不同，参考教程：  
- Solana 加池教程：[Solana 加池](/zh/sol/creatpool)  
- BSC/ETH 加池教程：[BSC/ETH 加池](/zh/createtoken/createliquidity)  
- Sui 加池教程：[Sui 加池](/zh/sui/pool)
...
- 各平台上传教程：[上传 logo 教程](/zh/question/logol)
...
- [标准预售](/zh/presale/simplemint)  
- [加池预售](/zh/presale/mintaddsale)  
- [捐赠预售](/zh/presale/donatemint)
...
- 市场 ID 教程：[Market 教程](/zh/sol/market)
...
1. **批量转账（空投）工具**：一次性向多个地址批量转账（BSC 一次最多支持 200 个地址）。  
   - BSC/ETH 批量转账教程：[批量转账](/zh/tools/batch-transfer)  
   - Solana 批量转账教程：[Solana 批量转账](/zh/sol/batch-transfer)

2. **代币持仓快照工具**：导出某个代币的持仓地址，便于针对性空投。  
   - BSC/ETH 快照教程：[代币快照](/zh/tools/snapshottoken)  
   - Solana 快照教程：[Solana 快照](/zh/sol/snapshottoken)

3. **市值管理 / 操盘工具**：导入多个地址，通过这些地址买卖实现价格管理（涨/跌）。  
   - EVM 市值管理（支持 BSC、Base、ETH 等）：[Swapbot 工具](/zh/createtoken/swapbot)  
   - Solana 市值管理：[Solana Swapbot](/zh/sol/swapbot)  
   - TRON 市值管理：[TRXBot](/zh/tron/trxbot)

4. **批量生成钱包工具**：快速生成数百个地址（支持 BSC、TRON、ETH、Solana 等）。  
   - 创建钱包工具：[生成钱包](/zh/tools/create-wallets)

5. **批量归集工具**：将分散在多个钱包的代币归集到一个地址。  
   - BSC/ETH 归集教程：[归集](/zh/tools/gather)  
   - Solana 归集教程：[Solana 归集](/zh/sol/gather)

6. **锁池 / 烧池工具**：用于锁定 LP 代币或烧池，防止项目方随意撤回流动性（增加用户信任）。  
   - 锁池教程：[锁池教程](/zh/createtoken/createlock)  
   - Solana 烧池教程：[烧池教程](/zh/sol/burn)

7. **靓号钱包生成**：生成带特定开头或结尾的 Solana 钱包地址（如带 888）。  
   - Solana 靓号生成教程：[靓号教程](/zh/sol/vanityAddress)

8. **增加流动性工具**：若池子太小，可使用流动性管理工具增加流动性。  
   - 增加/撤出流动性教程：[LP 管理](/zh/createtoken/lpmanage)  
