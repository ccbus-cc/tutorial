...
- 底池燃烧类型的代币可能存在被机器人/夹子恶意套利的风险，如不能接受，可发行标准币
...
链切换好之后，打开发币页面：[https://www.ccbus.cc/#/create-token/lp-burn](https://www.ccbus.cc/#/create-token/lp-burn) 点击右上角连接钱包
...
老规矩，我们在CCBus发币页面：[https://www.ccbus.cc/#/create-token/lp-burn](https://www.ccbus.cc/#/create-token/lp-burn)，填写相应的参数：

<img src="../.gitbook/assets/ccbus-lp-burn-create.png" alt="Mint+底池燃烧创建表单" />
...
随便填参数（不能填0），发币之后不要往合约地址里转币，直接加池开盘，就不会Mint
...
不需要的部分不能填空，必须填0，营销税除外，总比例要小于25%
...
| 参数 | 说明 |
|------|------|
| **营销钱包** | 如果底池是BNB池子，就获得BNB |
| **选择交易所** | 不同的链会有不同的交易平台。选择什么交易所，就必须去那里添加流动性。搞错了会导致机制无法执行，请注意 |
| **选择底池代币** | 只能选择`BNB`做池子 |
...
<img src="../.gitbook/assets/底池燃烧开关.jpg" alt="" />

| 开关 | 说明 |
|------|------|
| **手动开启交易** | **必须选它**。该功能机制必须选择手动开启交易 |
| **杀区块** | **选它**：用于防止机器人抢跑买入，杀3区块意思就是前3区块(BSC上单个区块3秒左右)买入的地址自动拉黑。**不选**：无法使用该功能，后期也不能再开启 |
| **税率开关** | **选它**：创建代币后手动调整税率，买卖税率各小于25%（该功能谨慎使用）。**不选**：创建代币后无法再修改滑点，后期也不能再开启 |
| **自动空投** | **选它**：每笔交易都会自动向随机地址空投小额代币，以增加持币效果，最多可空投5个地址。**不选**：无法使用该功能 |
| **黑名单功能** | **选它**：能够`添加`和`解除`黑名单。被拉入黑名单的地址将无法卖出代币，也不能转账，该功能慎用。**不选**：无法设置和解除黑名单 |


- 代币创建成功后，页面会自动跳转到代币设置页面（控制台），可以直接对代币进行管理操作
- 合约会自动开源，无需手动验证



在薄饼（PancakeSwap）或其他DEX第一次添加流动性时，**必须做V2的池子，不能做V3的池子**。V3不支持任何税率、燃烧等机制，只能在V2做，请务必注意！


## 5、控制台说明

当我们成功发行代币后，可进入控制台，对代币的各项功能进行管理。我们连接钱包之后，打开控制台，修改下列功能：

<img src="../.gitbook/assets/mint+底池燃烧控制台.jpg" alt="" />
...
<img src="../.gitbook/assets/Mint+地池燃烧复制控制台.png" alt="复制控制台链接" />
