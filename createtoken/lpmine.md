...
LP挖矿代币可能存在被机器人/夹子恶意套利的风险，如不能接受，可发行标准币。
...
链切换好之后，打开发币页面：[https://www.ccbus.cc/#/create-token/lp-mine](https://www.ccbus.cc/#/create-token/lp-mine) 点击右上角连接钱包
...
钱包连接成功后，我们通过CCBus可视化页面开启创建，打开 [https://www.ccbus.cc/#/create-token/lp-mine](https://www.ccbus.cc/#/create-token/lp-mine)，填写相应的参数：

<img src="../.gitbook/assets/ccbus-lp-mine-create.png" alt="LP挖矿+推荐奖励创建表单" />
...
不需要的部分不能填空，必须填0，营销税除外，总比例应该小于25%。
...
| **自动空投** | 每笔交易都会自动向随机地址空投小额代币，以增加持币效果，最多可空投5个地址。开启后**不可关闭、不可修改** | 无法使用该功能 |
| **黑名单功能** | 能够`添加`和`解除`黑名单。被拉入黑名单的地址将无法卖出代币，也不能转账，该功能慎用 | 无法设置和解除黑名单 |

## 5、创建成功后


- 代币创建成功后，页面会自动跳转到代币的**设置页面**（控制台），您可以在此管理代币的各项功能。
- 合约会自动进行**开源验证**，无需手动操作。


## 6、控制台说明

当我们成功发行代币后，可进入控制台，对代币的各项功能进行管理。我们连接钱包之后，打开 [https://www.ccbus.cc/#/create-token/lp-mine](https://www.ccbus.cc/#/create-token/lp-mine) 的控制台，修改下列功能：
...
* [x] **为什么没有挖矿奖励？**
  ...
  * 3、如果用户添加LP且过了24小时，但是整个代币没有交易，也是不能挖矿的。纯链上挖矿，必须有人交易出Gas费才可以。这个交易的人不一定是添加LP的人，其他新人买卖也算。如果代币交易不活跃，就很可能出现没有挖矿奖励
  * 4、用户每次获得挖矿奖励时间，都要与上一次奖励发放的时间间隔超过24小时才行。假设用户昨天晚上10点加了LP，那第二天10点后才能获得挖矿奖励。假设用户是第二天晚上11点获得挖矿奖励，那么等到第三天11点之后才能获得奖励。以此类推，用户获得挖矿奖励之后超过24小时，且在全网有交易的情况下，才能继续获得奖励。不到24小时，是没有奖励的
* [x] **用户加池撤池子的问题**
...
在薄饼（PancakeSwap）第一次添加流动性的时候，**必须做V2的池子，不能做V3的池子**。V3不支持任何机制，所以只能在V2做，请注意。同样，在 UniSwap 等其他DEX上也应选择 V2 流动性。
