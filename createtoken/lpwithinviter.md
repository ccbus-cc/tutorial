...
LP分红+推荐代币可能存在被机器人/夹子恶意套利的风险，如不能接受，可发行标准币。
...
<img src="../.gitbook/assets/小狐狸切换BNB.png" alt="" />

如果要在Base发币，就切换到Base链。要在以太坊发币，就切换到ETH链，这里就不演示了。

链切换好之后，打开发币页面：[https://www.ccbus.cc/#/create-token/lp-reflection-with-inviter](https://www.ccbus.cc/#/create-token/lp-reflection-with-inviter) 点击右上角连接钱包

<img src="../.gitbook/assets/钱包连接.png" alt="" />

之后会弹出小狐狸让你确认要连接的钱包地址

<img src="../.gitbook/assets/LP分红+推荐奖励连钱包.png" alt="" />

点击下一步并确认之后，就会连接成功了。在发币页面的右上角，会看到你的`链名称`和`钱包地址`，这就算完成了

<img src="../.gitbook/assets/LP分红+推荐奖励 链名称.png" alt="" />

## 3、参数说明

钱包连接成功后，我们通过CCBus可视化页面开启创建，打开 [https://www.ccbus.cc/#/create-token/lp-reflection-with-inviter](https://www.ccbus.cc/#/create-token/lp-reflection-with-inviter)，填写相应的参数：

<img src="../.gitbook/assets/ccbus-lp-with-inviter-create.png" alt="LP分红+推荐奖励创建表单" />
...
| **精度** | 代表币的小数位数，如 0.000001 代表精度为6。一般默认是18 |
...
不需要的部分不能填空，必须填0，营销税除外，总比例小于25%。解释同买入税率。


### 推荐奖励


所有代数比例相加必须等于 **100%**，才视为设置成功。如果不会计算，可直接点击`核算比例`，帮你自动核算成功。
...
| **选择交易所** | 不同的链会有不同的交易平台（如ETH链有UniSwap，BSC链有PancakeSwap等）。选择什么交易所，就去那里添加流动性。搞错了会导致机制无法执行，请注意 |
...
<img src="../.gitbook/assets/LP分红+推荐 开关.png" alt="" />

| 开关 | 选它 | 不选 |
|------|------|------|
| **手动开启交易** | 需要在控制台打开交易开关，才能够交易，并且打开后无法重新关闭 | 加池子后立即可以交易 |
| **杀区块** | 用于防止机器人抢跑买入，杀3区块意思就是前3区块（BSC大概9秒）买入的地址自动拉黑 | 无法使用该功能，后期也不能再开启 |
| **税率开关** | 创建代币后可手动调整税率，买卖税率各小于25% | 创建代币后无法再修改滑点，后期也不能再开启 |
| **自动空投** | 每笔交易都会自动向随机地址空投小额代币，以增加持币效果，最多可空投5个地址。该功能开启后**无法关闭、无法修改** | 无法使用该功能 |
| **黑名单功能** | 能够`添加`和`解除`黑名单。被拉入黑名单的地址将无法卖出代币，也不能转账，该功能慎用 | 无法设置和解除黑名单 |

## 5、创建代币


- 创建成功后页面会自动跳转到该代币的**设置页面**（控制台），你可以在那里管理代币的各项功能。
- 合约会**自动开源**，无需手动验证。
- 如果你选择了**放弃所有权**，合约会在创建时自动丢弃权限。
...
<img src="../.gitbook/assets/LP分红+推荐奖励控制台.png" alt="" />
...
* [x] **分红控制**
  ...
  * **设置分红阈值：**当分红钱包内的分红代币数量超过该值时，开始分红，默认为0.1个分红代币。设置过高可能会导致长时间无分红，强烈建议新用户不要修改此选项
* [x] **推荐奖励控制**
...
<img src="../.gitbook/assets/LP分红+推荐奖励 控制台地址.png" alt="复制控制台链接" />
...
在薄饼（PancakeSwap）或其他DEX第一次添加流动性的时候，**必须做V2的池子，不能做V3的池子**。V3不支持任何税率机制，所有的买卖税、分红、回流等功能在V3池子中均不生效，请务必注意。


<img src="../.gitbook/assets/选择V2 lp池子.png" alt="" />
